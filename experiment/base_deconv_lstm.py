...
class DeConvLSTMCell(DropoutRNNCellMixin, base_layer.BaseRandomLayer):
    ...
    def _conv_func(self):
        if self.rank == 1:
            return backend.conv1d_transpose
        if self.rank == 2:
            return backend.conv2d_transpose
        if self.rank == 3:
            return backend.conv3d_transpose
